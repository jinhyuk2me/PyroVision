...
sysShutterManual = lep_sys_shutter_mode(0, 0, 1, 0, 0, 180000, explicitCmdToOpenVal, desiredFfcTempDeltaVal,
                                        imminentDelayVal)
sysShutterAuto = lep_sys_shutter_mode(1, 0, 1, 0, 0, 180000, explicitCmdToOpenVal, desiredFfcTempDeltaVal,
                                      imminentDelayVal)
...
def print_device_formats(devh):
    for format_desc in uvc_iter_formats(devh):
        ...
        for frame_desc in uvc_iter_frames_for_format(devh, format_desc):
            print("  frame {0}x{1} @ {2}fps".format(frame_desc.wWidth, frame_desc.wHeight,
...
def set_manual_ffc(devh):
    ...
    res = set_extension_unit(devh, SYS_UNIT_ID, controlID, byref(sysShutterManual),
                             sizeData)  # set_extension_unit(devh, unit, control, data, size)
    print(res)
...
def set_auto_ffc(devh):
    ...
    controlID = (getSDK >> 2) + 1  # formula from Kurt Kiefer
    print('controlID: ' + str(controlID))
    res = set_extension_unit(devh, SYS_UNIT_ID, controlID, byref(sysShutterAuto),
                             sizeData)  # set_extension_unit(devh, unit, control, data, size)
    print(res)
...
def perform_manual_ffc(devh):
    ...
    print('controlID: ' + str(controlID))
    res = set_extension_unit(devh, SYS_UNIT_ID, controlID, shutter_mode, sizeData)
    print("response : ", res)
